...
for i in range(n):
    ...
    subprocess.Popen(["ssh", "-i", key_paths[i], f"{users[i]}@{node_addresses[i]}", f"cd /mnt/share/src/swiftpaxos_copy && go install -buildvcs=false && ~/go/bin/swiftpaxos -run server -config {config_file} -protocol {protocol} -alias {aliases[i]} -log ~/{aliases[i]}_"])